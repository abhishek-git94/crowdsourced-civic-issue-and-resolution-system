...
class Upvote(Document):
    meta = {
        'collection': 'upvotes',
        'indexes': [{'fields': ('user', 'issue'), 'unique': True}]
    }
...
class Notification(Document):
    """Per-user notification stored in MongoDB."""
    meta = {'collection': 'notifications'}

    user    = ReferenceField(User, required=True, reverse_delete_rule=CASCADE)
    type    = StringField(max_length=50, default='system')   # status|alert|success|system
    title   = StringField(max_length=200, required=True)
    ...
    def __repr__(self):
        return f"<Notification user={self.user.id} title={self.title[:30]}>"
